...
class Feed: NSObject, NSCoding {
    ...
    required convenience init?(coder aCoder: NSCoder) {
        let storedItems = aCoder.decodeObjectForKey("feedItems") as? [FeedItem]
        let storedURL = aCoder.decodeObjectForKey("feedSourceURL") as? NSURL

        guard let items = storedItems,
            url = storedURL else {
                return nil
        }

        self.init(items: items, sourceURL: url)
    }
